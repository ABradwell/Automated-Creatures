...
#desiredpoint2 is the right side
    ...
    if desiredpoint1 <  colorgridsize :
        if colorgrid[ desiredpoint1 ] == 1:
            groundtype = 'ground'
            run = True
        else :
            groundtype = 'water'
    ...
    while moverun == True:
        ...
        while run == False:
            ...
            if clicker == None :
                ...
                run = True
        while run == True:
            #Check if in the bush placement level
            clickerx = clicker.getX()
            clickery = clicker.getY()
            if hoverpointx < 100 and hoverpointy < 100 :
                placement(visuals, sizecolorgrid)
...
def placement (visuals, size, colorgrid):
    win.getMouse()
    busharray = []
    #
    cubewidth = visuals.getWidth()/size
    ...
    arraylength = size
    arrayheight = size
    #Finding where the mouse clicks
    mouseclick = win.getMouse()
    row = (mouseclick.getY() // size)
    collumn = (mouseclick.getX() // size)
    #dins whether land or water
    listplacement = (row*size + collumn)
    colorgridsize = (visuals.getWidth() // cubewidth) * (visuals.getHeight() // cubeheight)
    #begins to determine the placement
    if colorgrid[ listplacement ] == 1:
            groundtype = 'ground'
            ground = True
    else :
        groundtype = 'water'
        ground = False
    if run == True:
        imagepoint = (checkMouse.getX(), checkMouse.getY())
        bush = Image(imagepoint,"doggo40.png")

    #NEW ARRAY

        #Where the array must be filled
    rowlength = visuals.getWidth / cubewidth
    above = placement - rowlength
    below = placement + rowlength
    left = placement - 1
    right = placement + 1
    core = placement
    i = 0
...
##        #Current x1 - left side of squares location
##        mouseclick = win.getMouse()
##        placementy, placementx = mouseclick.getY(), mouseclick.getX()
##        #currenty = Text(Point(mousex, mousey), '+')
##        colorgridsize = (visuals.getWidth() // cubewidth) * (visuals.getHeight() // cubeheight)
##        yspotcube = (visuals.getHeight() // cubeheight)
##        xspotcube = (visuals.getWidth() // cubewidth)
##        desiredpoint1 = int(dw1 * size + currentx)
##        
##        #desiredpoint1 is the left side
##        #desiredpoint2 is the right side
